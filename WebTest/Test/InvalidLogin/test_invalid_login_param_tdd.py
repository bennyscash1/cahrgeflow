...
class TestInvalidLoginWeb:
    ...
    @pytest.mark.parametrize(
        "is_email_invalid, is_password_invalid",
        [
            (True, False),
            (False, True),
        ]
    )
    def test_invalid_login_param_tdd(self, is_email_invalid, is_password_invalid):
        email = (
            generate_email()
            if is_email_invalid
            else GetData.loaded_data[VarData.WebUserName]
        )
        password = (
            "Aa123"  
            if is_password_invalid
            else GetData.loaded_data[VarData.WebPassword]
        )
